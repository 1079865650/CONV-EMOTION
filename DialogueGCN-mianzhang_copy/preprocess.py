...
def split():
    ...
    #     test_vids = pickle.load(open('/home/rock/project/conv-emotion/data/erc-training/iemocap/iemocap_features_roberta.pkl', 'rb'), encoding='latin1')
    ...
    raw_data = pickle.load(open('/home/rock/project/conv-emotion/data/erc-training/iemocap/iemocap_features_roberta.pkl', 'rb'), encoding='latin1')
    print(f"😎 成功读取！新数据集里共有 {len(raw_data)} 个元素！")

    # 使用星号 *others 吸收掉多余的元素，我们只拿前 9 个需要的！
    video_ids, video_speakers, video_labels, video_text, \
        video_audio, video_visual, video_sentence, trainVids, \
        test_vids, *others = raw_data
